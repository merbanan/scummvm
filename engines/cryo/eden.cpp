...
void EdenGame::my_bulle() {
	...
	int16 wordWidth = 0;
	int16 lineWidth = 0;
	// A phrase can ask for the value of a game variable to be spelled out in the
	// middle of it, so characters don't always come straight from the phrase
	char number[8];
	...
	for (;;) {
		if (numberPtr) {
			...
			if (c == 0x11 || c == 0x13) {
				...
				// Spell out a byte variable, named by the byte which follows.
				// The panel's save and load lines number their slot this way.
...
// The movies a release without game data has to show, in the order its own
// player walked through them. DEMO.EXE plays a subset of these, and HNM.BAT
// hands the whole set to an external viewer one at a time; the two together
// come to this list. DINOLA1, which DEMO.EXE also names, is not on the disc.
...
void EdenGame::playMovieReel() {
	for (int i = 0; i < ARRAYSIZE(kMovieReel) && !_vm->shouldQuit(); i++) {
		if (!_graphics->playMovieFile(kMovieReel[i])) {
			// Most of these movies are packed with a scheme the decoder does
			// not handle yet, so skipping is the normal case for now
...
void EdenGame::run() {
	...
	// A release which is only a reel of movies has no resource file to open and
	// no game to set up: play the reel and be done
	...
	if (movieReel) {
		...
		while (!_quitFlag2) {
			...
			// A saved game named on the command line stands in for the intro. It
			// has to be read here rather than any earlier: initGlobals() has just
			// wiped everything it puts in place. Take the slot up only once, so
			// that finishing the game starts the next one from the beginning.
...
void EdenGame::startmusique(byte num) {
	...
	// The music carries its rate the way a VOC sound chunk does, as the divisor
	// the DOS driver loaded its timer with
	delete _musicChannel;
...
void EdenGame::noclicpanel() {
	...
	if (_curSpot2 >= &_gameIcons[119]) {
		...
		if (idx >= 0 && idx < kNumSaveSlots) {
			// A load line names the place its saved game was made in. The dialog
			// has a line per area and picks between them on the area number left
			// here, so hand it the one out of the save rather than the one being
			// played; the slot itself is spelled out of _var43.
...
// Read which of the game's twelve areas a saved game was made in, without
// loading it. The panel names the place in its load lines, and the dialog picks
// which of them to show by looking at the area number.
