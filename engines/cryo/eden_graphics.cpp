...
void EdenGraphics::stepRoomVideo() {
	...
	if (_roomVideo->endOfVideo()) {
		...
		// frames themselves being read as they are wanted.
	...
	// The frames come two to a picture: the even ones are its left half and the
	// odd ones its right, and they join without a seam. A room which scrolls is
	// that picture, both halves of it, which is what afsalle asks the decoder
	// twice for. A room which does not takes the left half alone.
...
void EdenGraphics::showMovie(int16 num, char arg1) {
	...
	do {
		...
		// blit and the screen update together take long enough that doing them
		// on every iteration pushes the next frame well past its due time,
		// which both slows playback down and starves the movie's sound
		if (newFrame)
			CLBlitter_CopyView2Screen(_hnmView);
		assert(_game->_vm->_screenView->_pitch == 320);

		// Pump events once per iteration, waiting no longer than the time left
		// until the next frame is due. Waiting past that point presents the frame
		// late, which starves the sound the movie carries and drags playback out;
		// polling twice per iteration doubled that wait.
...
// Play a movie that sits loose on the disc rather than inside the game's
// resource file, which is all a release without game data has to show. Returns
// false when the file isn't there or holds nothing we can decode.
bool EdenGraphics::playMovieFile(const Common::String &fileName) {
	...
	// These movies are not all the same height, and the screen stays the size it
	// is. Blank it so that whatever the movie does not cover reads as the black
	// bars the game draws above and below its own picture, rather than leaving
	// the previous movie showing around the edges of a shorter one.
